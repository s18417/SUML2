...
if option == "Wydźwięk emocjonalny tekstu (eng)":
    ...
    if text:
        ...
        st.write(answer)
if option == "Tłumacz anigelski - niemiecki":
    st.image(image)
    text1 = st.text_area(label="Wpisz tekst po angielsku żeby przetłumaczyć go na język niemiecki")

    if text1:
        translator = pipeline("translation_en_to_de")
        translated_json = translator(text1)
        argument = json.dumps(translated_json)
        resp = json.loads(argument)
        translated = resp[0]["translation_text"]
        if translated == text1:
            st.error("translacja zakończona niepowodzeniem, usuń literówki i upewnij się że tekst jest w języku angielskim")
        else:
